...
export const LuxuryView: React.FC<LuxuryViewProps> = ({
  ...
  const ownedAssets = luxuryAssets.filter((a) => a.owned);
  const totalLuxuryValue = ownedAssets.reduce((sum, a) => sum + a.cost, 0);
  const totalUpkeep = ownedAssets.reduce((sum, a) => sum + (a.upkeepPerSec || 0), 0);
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-slate-950/60 p-2 rounded-xl border border-slate-800/80">
                      ...
                      <div className="font-extrabold text-purple-300 flex items-center gap-1">
                        ...
                        <span>+{asset.prestigePoints} PTS</span>
                      </div>
                    </div>
                ...
                <div className="p-5 pt-0 border-t border-slate-800/60 mt-4 flex items-center justify-between gap-3">
                  ...
                  {asset.owned ? (
                    <button
                      ...
                      className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-rose-950/60 text-slate-300 hover:text-rose-400 border border-slate-700 hover:border-rose-500/40 text-xs font-bold transition"
                    >
                      Sotish (80%)
                    </button>
                  ) : (
                    ...
                    >
                      {canAfford ? 'Xarid Qilish' : 'Mavjud emas'}
                    </button>
                  )}
                </div>
              </div>
